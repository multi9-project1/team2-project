...
try:
    from app.logic.fashion_config import DATASET_STYLE_GROUP_LABELS, DATASET_STYLE_LABELS, FIT_PREFERENCE_TO_Q411_SCORE, STYLE_LABELS, TPO_OPTION_TO_DEEPLINK_KEYWORD
    ...
    from fashion_config import DATASET_STYLE_GROUP_LABELS, DATASET_STYLE_LABELS, FIT_PREFERENCE_TO_Q411_SCORE, STYLE_LABELS, TPO_OPTION_TO_DEEPLINK_KEYWORD
...
def get_recommendation_summary_text(request: RecommendationQueryRequest) -> Dict[str, Any]:
    try:
        ...
        result["text"] = result.get("preference_analysis", {}).get("text", "")
        result["recommendations"] = result.get("recommendation_results", [])
        return result
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
